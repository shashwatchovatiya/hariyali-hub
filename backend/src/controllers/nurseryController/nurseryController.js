...
exports.deleteNurseryDetail = async (req, res, next) => {
    try {
        ...
        const result = await nurseryModel.findOneAndDelete({ _id: req.nursery, user: req.user });
        ...
        // Sequential deletes for local setup
        await plantModel.deleteMany({ user: req.user, nursery: req.nursery });
        await nurseryStoresTabs.deleteMany({ user: req.user, nursery: req.nursery });
