...
exports.deleteNurseryStoreTab = async (req, res, next) => {
    try {
        //! Check for Authorization 
        if (!req.nursery || !req.role.includes('seller')) {
            const error = new Error("You are not allowed to access this route");
        ...
        const result = await nurseryStoreTabs.findByIdAndDelete(id);
        ...
        await nurseryStoresTemplate.deleteMany({ nurseryStoreTabs: id });

        //* FINDING AND DELETING BLOCKS....
        await nurseryStoresBlock.deleteMany({ nurseryStoreTabs: id });
        ...
        const info = {
            status: true,
            message: "Deleted Nursery Store Tab",
            result: {
...
exports.addNurseryStoreTemplates = async (req, res, next) => {
    try {
        ...
        if (getIndexForStoreTemplate) {
            await nurseryStoresTemplate.updateMany({
                user: req.user,
                nursery: req.nursery,
                nurseryStoreTabs,
                index: { $gte: index }
        ...
        await newNurseryStoreTemplates.save();
        ...
        const info = {
            status: true,
            message: "New store template is added to the nursery store tabs",
...
exports.editNurseryStoreTemplate = async (req, res, next) => {
    try {
        ...
        if (preData.index < 0 || nextData.index < 0 || Math.abs(nextData.index - preData.index) != 1) { 
...
exports.deleteNurseryStoreTemplate = async (req, res, next) => {
    try {
        //! Check for Authorization 
        if (!req.nursery || !req.role.includes('seller')) {
            const error = new Error("You are not allowed to access this route");
        ...
        //? FINDING AND DELETING TEMPLATES
        const result = await nurseryStoresTemplate.findByIdAndDelete(id);
        ...
        await nurseryStoresTemplate.updateMany({ nurseryStoreTabs: result.nurseryStoreTabs, index: { $gt: result.index } }, { $inc: { index: -1 } });

        //? FINDING AND DELETING ALL BLOCKS
        await nurseryStoresBlock.deleteMany({ nurseryStoreTemplate: id });
