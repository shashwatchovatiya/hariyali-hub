    ...
    const handelInitOrder = (payment) => {
      const orderData = {
          ...
          orderItems: checkoutCart.map(cart => ({
              ...
              images: (cart.plant.images && cart.plant.images.length > 0 && cart.plant.images[0].public_id) 
                ? cart.plant.images[0] 
                : { public_id: 'no-image-' + cart.plant._id, url: cart.plant.images && cart.plant.images[0] && cart.plant.images[0].url ? cart.plant.images[0].url : 'https://via.placeholder.com/550x650.png?text=No+Image' },
