...
const ProductSummary = () => {
    ...
    const handleAddToCart = () => {
        const nurseryId = product.nursery?._id || product.nursery;

        if (!nurseryId) {
            message.error("Plan nursery information is missing. Cannot add to cart.");
            return;
        }
    ...
    const handelBuyProduct = () => {
        ...
        if (!nurseryId) {
            message.error("Plan nursery information is missing. Cannot buy now.");
            return;
        }
        ...
        <>
            <div className="col-lg-3 p-4 mt-3 border">
                ...
                <p className="card-text">
                    ...
                    {
                        user && product.user && user._id === product.user ?
                ...
                <p className="card-text">
                    {
                        user ?
                            !(product.user && user._id === product.user) && <button onClick={handelBuyProduct} style={{ width: "100%" }} className='btn btn-success'>Order Now</button>
                            :
