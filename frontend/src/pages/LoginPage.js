...
const LoginPage = () => {
  ...
  const {
    ...
    email,
    isUserTwoFactorAuthNeeded,
    twoFactorAuthNeededToken,
  } = useUserLogin();
  ...
  if (user) {
    ...
    return (
      <Navigate to={`${redirect ? redirect : "/profile"}`} replace={true} />
    );
  }
  ...
  if (isUserVerificationNeeded) {
    let navigate = (
      <Navigate
        to={`/account/verificationEmail?email=${email}`}
        replace={true}
      />
    );
    return navigate;
  ...
  if (isUserTwoFactorAuthNeeded && twoFactorAuthNeededToken) {
    let navigate = (
      <Navigate
        to={`/account/twoFactorAuthentication/${twoFactorAuthNeededToken}`}
        replace={true}
      />
    );
    return navigate;
